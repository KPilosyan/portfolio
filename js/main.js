...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const strings = ["a Project Manager", "a Product Manager", "an Entrepreneur"];
    const typeSpeed = 70;
    const backSpeed = 20;
    ...
    function typeText(textElement, strings) {
        ...
        window.addEventListener("pageshow", function (event) {
            if (event.persisted) {
              // Force reload to restart animation properly
        ...
        form.addEventListener('submit', async (e) => {
          ...
          try {
            ...
            if (response.ok) {
              ...
              msg.style.color = '#4e844e';
              form.reset();
            } else {
              const data = await response.json();
              const errors = data.errors?.map(e => e.message).join(', ') || 'Oops! Please try again.';
